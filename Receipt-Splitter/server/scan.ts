...
- tip: actual tip amount paid, or null if the receipt does not show a tip
...
- Do not treat suggested tip percentages (for example 18%/20%/25%) as the tip amount
...
function getApiKey(): string {
  ...
  if (!apiKey) {
    throw new Error(
      'Receipt scanning is not configured. Add GEMINI_API_KEY to your .env file.',
    );
  }
  return apiKey;
}
...
export function parseReceiptPayload(raw: unknown): ParsedReceipt {
...
export async function compressReceiptImage(base64: string): Promise<{
  base64: string;
  mimeType: string;
}> {
...
export async function scanCompressedReceipt(
  base64: string,
  mimeType: string,
